...
class ClusterHttpApiSpec extends WordSpecLike with Matchers with ScalatestRouteTest with ClusterHttpApiJsonProtocol {
  ...
  "Cluster Http API" should {
    "return list of members" when {
      "calling GET /members" in {
        ...
        val uniqueAddress1 = UniqueAddress(address1, 1)
        val uniqueAddress2 = UniqueAddress(address2, 2)
        ...
        val unreachable = Map(
          UniqueAddress(address3, 2) -> Set(uniqueAddress1, uniqueAddress2)
        )
        ...
        Get("/members") ~> ClusterHttpRoute(mockedCluster) ~> check {
    ...
    "join a member" when {
      "calling POST /members with form field 'memberAddress'" in {
        ...
        Post("/members/", urlEncodedForm) ~> ClusterHttpRoute(mockedCluster) ~> check {
          responseAs[ClusterHttpApiMessage] shouldEqual ClusterHttpApiMessage("Operation executed")
          status == StatusCodes.OK
        }
      }
    }
    ...
    "return information of a member" when {
      "calling GET /members/akka://Main@hostname.com:3311" in {
        ...
        Get(s"/members/$address") ~> ClusterHttpRoute(mockedCluster) ~> check {
    ...
    "execute leave on a member" when {
      "calling DELETE /members/akka://Main@hostname.com:3311" in {
        ...
        Delete(s"/members/$address") ~> ClusterHttpRoute(mockedCluster) ~> check {
          responseAs[ClusterHttpApiMessage] shouldEqual ClusterHttpApiMessage("Operation executed")
          status == StatusCodes.OK
        }
      }
      ...
      "calling PUT /members/akka://Main@hostname.com:3311 with form field operation LEAVE" in {
        ...
        Put(s"/members/$address", urlEncodedForm) ~> ClusterHttpRoute(mockedCluster) ~> check {
          responseAs[ClusterHttpApiMessage] shouldEqual ClusterHttpApiMessage("Operation executed")
          status == StatusCodes.OK
        }
      }
      ...
      "does not exist and return Not Found" in {
        ...
        Put(s"/members/$address", urlEncodedForm) ~> ClusterHttpRoute(mockedCluster) ~> check {
          responseAs[ClusterHttpApiMessage] shouldEqual ClusterHttpApiMessage("Member not found")
    ...
    "return not found operation" when {
      "calling PUT /members/akka://Main@hostname.com:3311 with form field operation UNKNOWN" in {
        ...
        Put(s"/members/$address", urlEncodedForm) ~> ClusterHttpRoute(mockedCluster) ~> check {
          responseAs[ClusterHttpApiMessage] shouldEqual ClusterHttpApiMessage("Operation not supported")
          status == StatusCodes.NotFound
        }
      }
    }
